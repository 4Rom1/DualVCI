...
[2] Vibration—Rotation Interaction in Polyatomic Molecules. II. The
Determination of Coriolis Coupling Coefficients Janet Hawkins Meal and S. R.
Polo, The Journal of Chemical Physics 1956 24:6, 1126-1133 */
//
#ifndef Shared_H
#define Shared_H
#include "stdint.h"
...
#define MEBOHR2_TO_CM                                                          \
  6.62607004081E9 / (8 * PI * PI * 5.291772109217 * 5.291772109217 *           \
                     2.99792458 * 9.1093829140)
//#define MEBOHR2_TO_CM
//5807.049070575313504001047865451953183834319123869964251972560085559754
...
#define allocate_TabDb(arr, n)                                                 \
  arr = NULL;                                                                  \
  arr = new double[n];
#define allocate_TabInt(arr, n) arr = new int32_t[n];
...
#define DisplaySize(Size1)                                                     \
  printf("DimAct %u, DimRez %u, NNZAct %u, NNZRez %u \n", Size1.DimAct,        \
         Size1.DimRez - Size1.DimAct, Size1.NNZAct, Size1.NNZRez);
...
#define TIME_DIFFS(t1, t2) t2.tv_sec - t1.tv_sec
//
// Measure CPU time in seconds
#define TIME_CPUS(t1, t2) ((double)(t2 - t1)) / CLOCKS_PER_SEC;
//
#define FinalMessaj()                                                          \
...
struct LocalFF {
  ...
  // DualHPos[xx]. LFF[xx].Idx[ii] < 0 are indexes of non coupled force
  // constants: KFC.KijNCpld[dd][mm] with dd=-LFF.Idx[ii]/NMode,
  // mm=-LFF.Idx[ii]-dd*NMode; 0 <= LFF[xx].Idx[ii] < NPES and are indexes of
  // coupled force constants KFC.KijCpld[LFF[xx].Idx[ii]]. LFF.Idx[ii] >= NPES
  // are key numbers of the rotational coefficients
  // nl*NMode^3+nk*NMode^2+nj*NMode+ni+NPES with a unique corresponding
  // (ni,nj,nk,nl) LFF[xx].Idx[ii] are defined for ii in [0,LFF[xx].Num[
...
uint32_t GetSizeActMax2(int MaxEv, int MAXNCV, int NMode, int MaxQLevel,
                        ...
                        int SumTarget, uint32_t NIdx32, uint32_t NXDualHTrunc);
// Return the maximal size of the active space SizeActMax for a given memory
// Mem(in megabytes),
/*
...
uint32_t MaxContribAll(float *Vec, uint32_t *IndexAssign, int *TabScreen,
                       ...
                       const int NAdd, int MaxAdd, const float Trh,
...
// for enrichment in previous iteration (else 1) Vec[nn*SizeRezMax] contains the
// eigenvector number nn. Nadd = number of elments to add at next iteration per
// non converged eigenpairs NNotConv. MaxAdd = Limit of number of elments to add
// at next iteration. Thr = Threshold above which a coordinate will be selected
// for enrichment
//
...
//
double ScaleVect(float *Vec, int Iteration, SizeArray *Size,
                 uint32_t SizeRezMax, int NScreenTot);
//
