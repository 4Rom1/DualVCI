...
per non converged eigenpairs at each iteration, bounded by %d \n",
 ...
 and shrinking factor for the maximal number of indexes in the residual graph KNZREZ =  %.1f\n",
  ...
  if (PrintOut <= 1) {
    printf("\n***PrintOut must be > 1 and matrix saved from previous DVCI "
           "run***\n\n");
    FinalMessaj() return 0;
  }
  if (Memory <= 0) {
  ...
  char BasisOut[30] = "-FinalBasis.bin"; // Final basis file name if PrintOut =
                                         // 2
  char MatOut[30] = "-Matrix.bin"; // Binary file name with matrix coefficients
                                   // if PrintOut = 2
  ...
  if (!ReadMatCSCBin(FileMat, &NEV, &NCV, &SizeInit, &DimAct, &NNZAct, &Shift,
                     &tol, &Ull, IJAct, ValAct)) {
    FinalMessaj() return 0;
  }
  ...
  InitTab(workd, 3 * DimAct + 1) double *workl = NULL;
  ...
  InitTabInt(ZeroSt, NMode) int *PositionTarget =
      new int[NTargetStates + 1]; // For zero
  if (GroundState <= 0) {
  ...
  int nconv = SolverCSCSym(Size, 0, NEV, NCV, Shift, Tol, EigVal, EigVec, workd,
                           workl, IJAct, ValAct);
  if (!nconv) {
  ...
  if (nconv) {
    ...
    int detect = 1;     // If it remains positive one of the Target has been
                        // detected
    long PosZero =
    ...
    if (FileRef != NULL) {
      ...
      for (int PosEig = 0; PosEig < nconv; PosEig++) {
        ...
        detect = 1;     // If it remains positive one of the Target has been
                        // detected
        PosZero = -1; // Position of groundstate if detected (might be not equal
                      // to zero)
        if (GroundState <= 0) {
          DetectZero = DetectMaxCoordinate(&EigVec[PosEig * Size[1].DimAct],
        ...
        if (NTargetStates && !DetectZero) {
          detect = DetectMaxCoordinate(&EigVec[PosEig * Size[1].DimAct],
                                       PositionTarget, NTargetStates, ThrCoor);
        }
        if (((EigVal[PosEig] - Ground <= MaxFreq) &&
             (EigVal[PosEig] - Ground >= MinFreq)) &&
            (detect || DetectZero)) {
          // Assignment display
          AfficheNuSupTex(ModeAct, &EigVec[PosEig * Size[1].DimAct], NMode,
                          Size[1].DimAct, ThrCoor);
          if (PosEig == PosZero) {
            if (!DoRot) {
              printf(" | %.2f(%d) | %.4f // \n \n", Ground, PosEig,
            ...
            } else // Add Watson
            {
              printf(" | %.2f(%d) | %.4f  | %.4f (Watson) // \n \n", Ground,
                     PosEig, GetClosest(TabRef, Ground) - (Ground),
                     Ull); // Add watson
            }
          } else {
