...
//DualHPos[Idm(xx)+mm] is defined for (xx,mm) in [0,NXDualHPlus[ x [0,NMode[.
 ...
 uint32_t xx;
//
 for (uint32_t ee=1; ee < NXDualHTruncPos; ee++) //First excitation = to zero
 {
...
{
//Return the number of positive excitation in function of NMode, NEccit(Couplings) 
//and Maxd degree of excitation : Sum_{i in [1,NEccit]} e_i <= Maxd
uint32_t CmptEct=0;
     for (int ee=NEccit;ee<= Maxd;ee++)
     {
     CmptEct+=(uint32_t)( factorial(ee-1)/(factorial(NEccit-1)*factorial(ee-NEccit)) );
     }
     //Number of NEccit-uplets = (NEccit among NMode)
     uint32_t NSurf=(uint32_t)(factorial(NMode)/(factorial(NMode-NEccit)*factorial(NEccit)));   
...
//Number of NEccit-uplets = (NEccit among NMode)
     NSurf=(uint32_t)(factorial(NMode)/(factorial(NMode-NEccit)*factorial(NEccit)));
//   
...
uint8_t *MultiSurf;
//
allocate_TabUint8(MultiDegrees, NEccit)
allocate_TabUint8(MaxDegrees, NEccit)
allocate_TabUint8(MultiSurf, NEccit)
MaxDegSurf=new uint8_t[NMode];
//
//Initialsize degrees
   uint8_t Sum;
//
...
//defined for XPolSupPos[Idm(xx)+mm], (xx,mm) in [0,NGenPoz[ x [0,NMode[.
   ...
   //double and Triple and superior excitation
    for(int ee=2;ee<=NCPol;ee++)
     {
        SizeGenOld=GeneratorXPoz (NMode, ee, XPolSupPos, SizeGenOld, DegreEx[ee-1]);
        if(!SizeGenOld){return 0;}      
     }
    return SizeGenOld;
  }
//
...
And at the same time the incomplete matrix vector product Hsb*X is calculated on the fly. Additionally it computes the graph of the residual matrix
stored in IJRez.
...
DegrePol : Maximal degree of PES.
Omega : harmonic frequencies in hartree for conversion of rotational elements.
QQ[ii].Coeff[ii][jj] are matrix elements of operator Q^ii for ii in [0,DegrePol],
...
Size[Iteration+1].DimAct: Current size of active space B already incremented at previous iteration;
Size[Iteration].DimAct: Previous size of active space B used as a starting index in the general loop;
Size[Iteration+1].NNZAct: Current number of NNZ of active matrix Hb already incremented in routine AssembleHarmCSC;
Size[Iteration+1].NNZRez: NNZ of residual matrix beeing incremented because DoGraph # 0;
*/
...
int CptQSort=0;
long LinAct=0;
int RestQSort=0;//Rest of division of Size[Iteration+1].DimRez by MaxQsortSeq
int KQSort=0;
int Cpld;
double MatElem;
uint32_t xx;
...
//At next iteration residual space has got minimum Size[Iteration].DimRez elements
//without the nulified elements handled in routines AssmbleHR.
Size[Iteration+1].DimRez=Size[Iteration].DimRez;
...
//having the number Lin.  
//Search Tester in ModeAct[PermutAct[1:Size[Iteration+1].DimAct]]   
 LinAct=QsearchMode(Tester, ModeAct,PermutAct,Size[Iteration+1].DimAct, SizeBit);
 if(LinAct<0)//It has to be negative to built residual space and vectors
  {  
...
//Useless if we cross new active space with residual one 
    if((LinRez<0))//From previous iteration : Size[Iteration+1].DimRez has been incremented
    {
...
//
        if(RestQSort==0 && KQSort)
         {
         QuickSortModeShift(ModeRez, 0,MaxQSortSeq-1,&PermutRez[(KQSort-1)*MaxQSortSeq],\
        SizeBit,(KQSort-1)*MaxQSortSeq);
//MaxQSortSeq elements are sorted each time Size[Iteration+1].DimRez is a multiple of the latter. 
         }      
     }      
      LinRez=-1;
      if(KQSort)
        {               
         ...
         while(LinRez<0 && CptQSort<KQSort)
          {
         //Search Tester in ModeRez[PermutRez[CptQSort*MaxQSortSeq : (CptQSort+1)*MaxQSortSeq]+CptQSort*MaxQSortSeq]
         LinRez=QsearchModeShift(Tester, ModeRez, &PermutRez[CptQSort*MaxQSortSeq],MaxQSortSeq,\
         SizeBit,CptQSort*MaxQSortSeq);
         CptQSort++;
          }
        }
//
...
//""
      ...
      (EigVec[TabScreen[ll]*Size[Iteration+1].DimAct+Lin]);   
            ...
            IJRez.I[Size[Iteration+1].NNZRez]=Size[Iteration+1].DimRez; 
//             
...
//
       ...
       {
       printf("**|Bs| Too Big : To poursue,increase Memory or KNREZ**\n");
       CheckOutputPolyX=0;
       goto end;
       }//Keep   
     else if(Size[Iteration+1].NNZRez >= SizeMax.NNZRez)
       {
       printf("**NNZ(Hsb) Too Big : To poursue,increase Memory or KNZREZ**\n");  
       CheckOutputPolyX=0;
       goto end;     
       }         
      }
  else if(LinRez>=0)
      {
        for (int ll=0; ll < NScreen; ll++)
        {  
        RezVect[LinRez+SizeMax.DimRez*ll]+=MatElem*\
        EigVec[TabScreen[ll]*Size[Iteration+1].DimAct+Lin];
        }
       if( Size[Iteration+1].NNZRez < SizeMax.NNZRez )   
       {
             if(AssignFirstCol)//Check if elements Lin of IJRez.NJ has already been set
             {//Remember that IJRez.NJ[Lin] give the number of NNZ for the first element of line Lin
             IJRez.NJ[Lin]=Size[Iteration+1].NNZRez;
             AssignFirstCol=0;
             } 
...
Size:
Size[Iteration+1].DimRez: Size of residual space beeing incremented;
Size[Iteration+1].DimAct: Current size of active space already incremented at previous iteration;
Size[Iteration+1].NNZAct: Current number of NNZ of active matrix already incremented in routine AssembleHarmCSC;
Size[Iteration+1].NNZRez: NNZ of residual matrix not beeing incremented because DoGraph=0;
*/
int DegrePolP1=DegrePol+1;
uint32_t Lin=0;
long LinRez=-1;//Important to evaluate RestQSort at first
