...
[1] R. B. Lehoucq, D. C. Sorensen, C. Yang, ARPACK Users ’ Guide : Solution of
Large Scale Eigenvalue Problems with Implicitly Restarted Arnoldi Methods,
Communication 6 (1998) 147. doi:10.1137/1.9780898719628.

Additionally to the original source provided, a description of DSAUPD can be
found here http://www.caam.rice.edu/software/ARPACK/UG/node136.html.
*/
...
int SolverCSCSym(SizeArray *Size, int Iteration, int NEV, int NCV, double Shift,
                 double tol, double *EigVal, double *EigVec, double *workd,
                 double *workl, CSC IJ, double *ValAct) {
  // Compute the eigenvalues and eigenvectors of matrix Hb stored in CSC format
  // (IJ,ValAct), and store eigenvectors using column major order. CSC :
  // compressed sparse column format: IJ.NJ[jj] = number of the first nnz
  // elmement of column jj IJ.I[nn] = line number of nnz element number nn
  ...
  // Hb=Hb-Shift*Id workd, workl, tol, NEV, NCV refer to the same variables
  // defined in ARPACK see the header file solver2.h or
  // http://www.caam.rice.edu/software/ARPACK/UG/node136.html for more details.
  //-----------------------------------------------------------------------
  //  %-----------------------------%
  ...
  InitTabInt(resid, Ndim) int *select = NULL;
  select = new int[NCV];
  int iparam[12] = {0};
  int ipntr[12] = {0};
  //  %---------------%
  ...
  int ido = 0, lworkl = 0, info = 0, ierr = 0, nconv = 0, maxitr = 0, mode = 0,
      ishfts = 0, Iterate = 0;
  int rvec = 0;
  double sigma = 0;
  ...
  do {
   ...
   &                 lworkl, info )*/
    ...
    dsaupd_(&ido, &bmat, &Ndim, whichf, &NEV, &tol, resid, &NCV, &EigVec[1],
            &Ndim, &iparam[1], &ipntr[1], &workd[1], &workl[1], &lworkl, &info);
    if ((ido == -1) || (ido == 1)) {
      //        %--------------------------------------%
      ...
      //        | workd(ipntr(2)).                     |
      //        %--------------------------------------%
      SparseSymCSC(&workd[ipntr[1]], &workd[ipntr[2]], IJ, ValAct,
                   Size[Iteration + 1]);
      //        %-----------------------------------------%
  ...
  if (info < 0) {
    ...
    dseupd_(&rvec, &HOWMNY, select, &EigVal[0], &EigVec[0], &Ndim, &sigma,
            &bmat, &Ndim, whichf, &NEV, &tol, &resid[0], &NCV, &EigVec[1],
            &Ndim, &iparam[1], &ipntr[1], &workd[1], &workl[1], &lworkl, &ierr);
    //     %----------------------------------------------%
...
void SparseSymCSC(double *X, double *Y, CSC IJ, double *Val, SizeArray Size) {
  ...
  for (uint32_t nn = 0; nn < Size.DimAct; nn++) {
    Y[nn] = 0;
  }
...
uint32_t ShiftDiagCSC(SizeArray Size, CSC IJ, double *ValAct, double Shift) {
  ...
  // IJ.NJ[jj] = number of the first nnz elmement of column jj
  // IJ.I[nn] = line number of nnz element number nn
  // Digonal elements of ValAct[nn] are such that IJ.I[nn]=jj for column jj in
  // [0,Size.DimAct[.
  uint32_t Cmpt1 = 0;
  for (uint32_t jj = 0; jj < Size.DimAct; jj++) {
    for (uint64_t nn = IJ.NJ[jj]; nn < IJ.NJ[jj + 1]; nn++) {
...
int ConvertCharToFortran(char *StringInC, char *StringInF) {
  // Change the termination of string in c ('\0') by a blank for fortran
  // equivalence
  int mon_i = 0;
