...
int SubStringChar(const char *str, char *new_s,
                  char C) { // Read a string until C and copy it into news
  int jj = 0;
  while ((str[jj] == ' ')) {
    jj++;
  };
...
int GetKeyWords(FILE *file1, int *NMode, int *DoRot, int *MaxEv, int *DeltaNev,
                int *AddTarget, int *PESType,
                uint8_t TargetState[MaxTarget][MaxNormal], int *DoGraph,
                int *MAXNCV, int *MaxQLevel, int *NAdd, int *MaxAdd,
                int *PrintOut, int *EvalDeltaE, double *EpsRez, double *KNREZ,
                double *KNNZ, double *KNZREZ, double *EtaComp, double *Tol,
                double *Kappa, double *ThrMat, double *ThrPES, double *ThrCoor,
                double *GroundState, double *MinFreq, double *MaxFreq,
                double *Freq0Max, float *ThrKX, double *Memory, int *Verbose,
                char *OutName, char *PESName, char *RefName) {
  ...
  while (fgets(chaine, MaxChar, file1) != NULL) {
    ...
    if (strcasecmp(chainedat, "NMode") == 0) {
      ...
      InitMat(TargetState, MaxTarget, *NMode) DetNC = 1;
    ...
    } else if (strcasecmp(chainedat, "EvalDeltaE") == 0) {
      sscanf(&chaine[curs], "%d", EvalDeltaE);
    ...
    if ((strcasecmp(chainedat, "TargetState") == 0) && DetNC == 1) {
      ...
      if (strncasecmp(&chaine[curs + BI], "Fund", 4) == 0) {
        ...
        for (int cc = 0; cc < NCom; cc++) {
          ...
          sscanf(&chaine[PoCom[cc] + 1 + BI], "%d%c%d", &Pair[0], &Pthesis,
                 &Pair[1]);
...
void GetKijMonm(FILE *file, double KijCpld[], double **KijNCpld, int32_t **Monm,
                int NMode, double ThrPES) {
  /*
  This routine read the PES file and store the coupled terms in KijCpld, and the
  non coupled in KijNCpld. KijNCpld[kk][mm]= non coupled force constants of
  degree kk+1 on normal coordinate mm < NMode. Monm[kk][mm] degree of monomial
  term kk on coordinate mm in PES. ThrPES : threshold for acceptable force
  constants.
  */
  char chaine[MaxChar] = {0};
...
long GetNumKijCpld(FILE *file, int NMode, double ThrPES, int *NCPol,
                   int *DegrePol, int DegreCoupl[MaxCpld]) {
  ...
  // Maximal degrees and couplings are respectively returned in  DegrePol and
  // NCPol ThrPES : threshold for acceptable force constants.
  char chaine[MaxChar] = {0};
  char subchaine[MaxChar] = {0};
  int cnt = 0;
  int curs1 = 0; // Position of the cursor
...
double AdjustCoeff(double **KijNCpld, double *KijCpld, int DegrePol, int NPES,
                   int PESType, int32_t **Monm, int NMode) {
  /*
  Routine that transformes all the force constant or derivative in cm-1
  depending on the value of PEStype. NPES : Number of coupled PES coefficients
  stored in KijCpld. Monm[kk][mm] : degree of monomial term kk of coordinate mm
  in PES. KijNCpld[kk][mm] : Non coupled force constants of degree kk+1 on
  normal coordinate mm. For PESType = 0 the force constants are already in cm-1.
  For PESType > 0 the derivatives are in atomic units.
  The conversion in cm-1 is done by setting q_i=Q_i / Omega[i],
  Omega[i]=sqrt(\nu_i).
  */
  int ii = 0;
  int jj = 0;
  ...
  if (PESType) // Derivatives in a.u
  {
    ...
    for (jj = 0; jj < NMode; jj++) {
      ...
      for (ii = 2; ii < DegrePol; ii++) {
        ...
        KijNCpld[ii][jj] =
            (KijNCpld[ii][jj] * HA_TO_CM) / (KFact * pow(Omega[jj], ii + 1));
      }
    }
    for (kk = 0; kk < NPES; kk++) {
...
int ReadCoor(FILE *file, double *CoorCart, double *Mas, double **CoorMode,
             int NAtome, int NMode, int Verbose) {
  // Read cartesian coordinates of equilibrium geometry in Bohr, masses in me
  // (electron rest mass) and normal coordinate vectors. The equilibrium
  // geometry in cartesian coordinates reads:
  //         X                       Y                      Z
  // CoorCart[3*(aa)],  CoorCart[3*(aa)+1],  CoorCart[3*(aa)+2], for aa
  // (0<=aa<NAtome)
  ...
  while ((fgets(chaine, MaxChar, file) != NULL) && Go) {
    ...
    if (!((DetectChar(chaine, '/', MinChar)) ||
          (DetectChar(chaine, '@', MinChar)))) { // Used for comment lines
      ...
      if ((DetectChar(chaine, '!', MinChar))) {
        ...
        if (CntExclam == 1) {
          sscanf(chaine, "%lf %lf %lf", &CoorCart[3 * (CntLine)],
                 &CoorCart[3 * (CntLine) + 1], &CoorCart[3 * (CntLine) + 2]);
          if (Verbose) {
            printf("%s", chaine);
          }
          CntLine++;
          ...
          sscanf(chaine, "%lf %lf %lf",
                 &CoorMode[(CntExclam - 3)][3 * (CntLine - StartNC)],
                 &CoorMode[(CntExclam - 3)][3 * (CntLine - StartNC) + 1],
  ...
  if (CntExclam == 0) {
    printf("\n****Problem reading coordinates-> exclamation mark should "
           "precede a field****\n\n");
    return -1;
  }
...
int GetDataPES(FILE *file, double KijCpld[], double **KijNCpld, int32_t **Monm,
               double *CoorCart, double *Mas, double **CoorMode, int NMode,
               int NAtome, int PESType, int Verbose, double ThrPES) {
  // Detect Forcefield or derivatives in section FORCEFIELD of PES file.
  // Also detect equilibrium geometry and normal coordinates in section
  // COORDINATE (if non void) Monm[kk][mm] degree du terme kk coordonnee mm in
  // PES, Everything is transformed in cm-1 depending on the value of PEStype.
  // For PESType = 0 the force constants are already in cm-1.
  // For PESType > 0 the derivatives are in atomic units.
  // KijNCpld[kk][mm]=Non coupled force constants of degree kk+1 on normal
  // coordinate mm.
  char chaine[MaxChar] = {0};
  int DoRot = 0;
  int Check = 1;
...
void GetKijMonm0(FILE *file, double KijCpld[], double **KijNCpld,
                 int32_t **Monm, int NMode, double ThrPES) {
  /*
  This routine read the PES file and store the coupled terms in KijCpld, and the
  non coupled in KijNCpld. Monm[kk][mm] degree of monomial term kk on normal
  coordinate mm in PES, KijNCpld[kk][mm]= non coupled force constants of degree
  kk+1 on normal coordinate mm < NMode.
...
long GetNumKijCpld0(FILE *file, int NMode, double ThrPES, int *NCPol,
                    int *DegrePol, int DegreCoupl[MaxCpld]) {
  ...
  for (a = 0; a < 7; a++) {
    for (b = 0; b < len; b++) {
      if ((ind = Chaine[b] - ' ') < 0)
        ind = 0;
      for (c = 0; c < 7; c++) {
        line[b * 8 + c] = glyyphs[(ind / 8 * 7) + a][(ind % 8 * 7) + c] == '@'
                              ? ind + ' '
                              : ' ';
      }
      line[b * 8 + 7] = ' ';
...
uint32_t GetConfsBin(FILE *FileBasis, uint8_t *&FinalBasis, int NMode,
                     int *NScreen) {
...
void PrintConfsBin(char *OutBasis, uint8_t *FinalBasis, int NMode, int NScreen,
                   uint32_t FinalSize) {
  // Print configurations of final basis set stored in FinalBasis into files
  // with extension name OutBasis, Files to write Final basis set (FileBasis)
  FILE *FileBasis = NULL;
  ...
  // Set up the ground state as the first binary eigenvector, no matter if
  // amoung targets or not Copy configuration arrays in binary format
  for (uint32_t ll = 0; ll < FinalSize; ll++) {
...
void PrintVecBin(char *OutVec, double *EigVec, double *EigVal, int NScreen,
                 int *TabScreen, int PosZero, double GroundState,
                 uint32_t FinalSize) {
  // OutBasis, binary eigenvector are also printed out into file having
  // extension OutVec. Files to write eigenvectors in binary format (FileVec)
  FILE *FileVec = NULL;
  FileVec = fopen(OutVec, "wb");
  // First the integer giving the number of screened states.
  // Set up the ground state as the first binary eigenvector, no matter if
  // amoung targets or not
...
void PrintMatCSCBin(char *OutMat, SizeArray *Size, int Iteration, int NEV,
                    int NCV, uint32_t SizeInit, double Shift, double tol,
                    double Ull, CSC IJ, double *ValAct) {
  ...
  // Size[Iteration+1].NNZAct is the number of NNZ of active matrix Hb at
  // current iteration. Size[Iteration+1].DimAct is the size of active space B
  // at current iteration. ValAct is an array of size Size[Iteration+1].NNZAct
  // containing NNZ of active matrix Hb The matrix will be shifted as
  // Hb=Hb-Shift*Id Ull Watson : -1/8*(Sum mu_ll) correction to add to
  // groundstate value
  FILE *FileMat = NULL;
...
int ReadMatCSCBin(FILE *FileMat, int *NEV, int *NCV, uint32_t *SizeInit,
                  uint32_t *DimAct, uint64_t *NNZAct, double *Shift,
                  double *tol, double *Ull, CSC &IJ, double *&ValAct) {
  // Print matrix Hb stored in CSC format (IJ,ValAct),
  // CSC : compressed sparse column format:
  // IJ.NJ[jj] = number of the first nnz elmement of column jj
  // IJ.I[nn] = line number of nnz element number nn
