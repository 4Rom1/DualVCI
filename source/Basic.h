...
//
template <typename T> int find_minimum(T *MonTab, int n) {
  int index, c;
  T min;
  min = MonTab[0];
  index = 0;
...
template <typename T> T MinimumVal(T *MonTab, int n) {
  int c;
  T min;
  min = MonTab[0];
  for (c = 1; c < n; c++) {
...
//
template <typename T> int find_maximum(T *MonTab, int n) {
  int index, c;
  T max;
  max = MonTab[0];
  index = 0;
...
//
template <typename T> T MinVal(T *MonTab, int n) {
  int c;
  T min;
  min = MonTab[0];
  for (c = 1; c < n; c++) {
...
//
template <typename T> T MaxVal(T *MonTab, int n) {
  int c;
  T max;
  max = MonTab[0];
  for (c = 1; c < n; c++) {
...
template <typename T> void SortAsc2(T *MonTab, long DimArray) {
  long ii, jj;
  T tempd;
...
template <typename T> void SortAsc3(T *MonTab, int DimArray) {
  int ii, jj;
  T tempd;
...
template <typename T> void SortDsc(T *MonTab, int *Permuter, int DimArray) {
  int ii, jj, tempi;
  T tempd;
...
template <typename T> void SortDsc2(T *MonTab, long DimArray) {
  long ii, jj, tempi;
  T tempd;
  for (ii = 0; ii < (DimArray - 1); ii++) {
    for (jj = 0; jj < (DimArray - 1 - ii); jj++) {
...
void SortAbsDsc(T *MonTab, long *Permuter, long DimArray) {
  ...
  for (ii = 0; ii < (DimArray - 1); ii++) {
    for (jj = 0; jj < (DimArray - 1 - ii); jj++) {
      if (SIGN(MonTab[jj]) * MonTab[jj] <
          SIGN(MonTab[jj + 1]) * (MonTab[jj + 1])) {
        tempd = MonTab[jj + 1];
        MonTab[jj + 1] = MonTab[jj];
        MonTab[jj] = tempd;
        tempi = Permuter[jj + 1];
...
//
template <typename T> long FindMaximum2(T *MonTab, long n) {
  long index, c;
  T max;
  max = pow(MonTab[0], 2);
  index = 0;
...
//
template <typename T> T FindMaximumVal(T *MonTab, int n) {
  int index, c;
  T max;
  max = pow(MonTab[0], 2);
  index = 0;
...
//
template <typename T> T Powo(T a, int n) {
  T Val = 1;
...
template <typename T> T FindMaximumValPoz(T *MonTab, int n) {
  int index, c;
  T max;
  max = pow(MonTab[0], 2);
  index = 0;
...
//
template <typename T> long FindMaximumAbs(T *MonTab, long n) {
  long index, c;
  T max;
...
//
template <typename T> long FindMaximumInd(T *MonTab, long n) {
  long index, c;
  T max;
  max = MonTab[0];
  index = 0;
...
//
template <typename T> int EqualArray(T *MonTab, T *b, int taille) {
  //
  int mon_i = 0;
...
//
template <typename T> T sum_array(T MonTab[], int num_elements) {
  int i;
  T sum = 0;
...
//
template <typename T> T SumPM(T MonTab[], int num_elements) {
  int i;
  T sum = 0;
  for (i = 0; i < num_elements; i++) {
...
//
template <typename T> T sum_array_abs(T MonTab[], int num_elements) {
  int i;
  T sum = 0;
  for (i = 0; i < num_elements; i++) {
