...
for (int mm=0; mm<  NMode ; mm ++) 
  {   
    if(PESType)
    {
     ...
     Pid[mm]=Min<uint32_t>((uint32_t)ceil(Freq0Max/(pow(Omega[mm],2)*HA_TO_CM))+2,MaxQLevel);
    }
    else
    {
     Omega[mm]=2*KijNCpld[1][mm];
     printf(" \\nu_{%d} : %.1f cm-1, ",mm+1,Omega[mm]);
     Pid[mm]=Min<uint32_t>((uint32_t)ceil(Freq0Max/Omega[mm])+2,MaxQLevel);  
...
//        
   ...
   //Allocation for generator XPolSupPos[xx].Degrees[mm], (xx,mm) in [0,NGenPoz[ x [0,NMode[
    ...
    KFC, XPolSupPos, NLFFPerEx, ZetaXYZ, ThrPES);
//
...
//
 ...
 KFC, DualHPos, LFF, ZetaXYZ, ThrKX, ThrPES, PermutGen, Corres);
// 
 if(Verbose)
 {
        ...
        if( ((EigVal[PosEig]-Ground <= MaxFreq+DeltaFreq) && (EigVal[PosEig]-Ground >= MinFreq)) && (detect || DetectZero) )
         {           
          ...
          if(AbsVal>ThrCoor)
           ...
           if(!ll)//Only print for the first showed state to avoid many copies of same vectors
