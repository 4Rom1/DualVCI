...
// force field, also say if the harmonic energy should be added to diagonal
// elements (IncK2=0) or not (IncK2=1)
...
https://doi.org/10.1016/j.cpc.2018.07.008\n");
  ...
  int DegrePol = 0; // Maximal degree in PES : not given as input will be set
                    // after reading of PES
  int DoRot = 1;    // If > 0 then Coriolis terms are computed
  // with non mass weighted normal coordinates for odd and mass weighted for
  // even
  ...
  double EtaComp =
      1.5; // New basis set will be selected from the maximal component
  // of error vector divided by EtaComp*NNotConv, where NNotConv is the number
  // of non converged eigenvectors at a given iteration
  ...
  int AddTarget = 2;    // Number of additional residual vectors to correct the
                        // potential increasing of targets
  int DegreEx[MaxCpld] = {0};  // Degree of excitation in each direction for the
                               // generator used here only to show values
  char PESName[MaxChar] = {0}; // Name of file of PES to be read.
  char RefName[MaxChar] = {
      0}; // Reference txt file containing frequencies in increasing order
  int MaxEv =
      30; // Maximal number of wanted eigenvalues for the Arnoldi eigensolver
  int DeltaNev = 1000; // Adjustment number for MaxEV that will be equal to
                       // Min(MaxEV,  MaxScreen+DeltaNev)
  // where PosEig is the eigenvalue position of maximal target
  int NCPol = 0; // Maximal couplings in polynomial: not given as input will be
                 // set after reading of PES.
  double Memory = 0; // Total allocated Memory in Megabytes
  int PrintOut =
      0; // If >0 print final basis set and eigenvectors for post treatment
  ...
  float ThrKX =
      1; // Threshold for contributions of sum force constants in dual operator
  double ThrPES =
      DBL_EPSILON; // Threshold for derivative or force constants of the PES
  ...
  /*
  NXDualHTruncPos : Total number of raising excitations for the dual of H (after
  truncation through ThrKX) NXDualHTrunc : Total number of excitations for the
  dual of H (after truncation through ThrKX)
  ...
  */
  int DegreCoupl[MaxCpld] = {
      0}; // Maxdegree of PES for each coupling: starts at 0 for coupling 1
  int DoGraph = 1; // If not equal to zero, it will store the graph of residual
                   // matrix in CSC format
  int EvalDeltaE =
      0; // If positif add correction vector and correction energy to targets
         // to gain computational time and then significantly increases memory
         // requirement
  printf("\n*****Reading Datas*****\n");
  int NTargetStates = GetKeyWords(
      FileKey, &NMode, &DoRot, &MaxEv, &DeltaNev, &AddTarget, &PESType,
      TargetState, &DoGraph, &MAXNCV, &MaxQLevel, &NAdd, &MaxAdd, &PrintOut,
      &EvalDeltaE, &EpsRez, &KNREZ, &KNNZ, &KNZREZ, &EtaComp, &Tol, &Kappa,
      &ThrMat, &ThrPES, &ThrCoor, &GroundState, &MinFreq, &MaxFreq, &Freq0Max,
      &ThrKX, &Memory, &Verbose, OutName, PESName, RefName);
  int NAddVar = NAdd;
  int NAddIter = NAdd;
  if (NMode == 0) {
  ...
  if (ThrPES > 0 && ThrMat > 0) {
    ...
    printf("\n Threshold for PES ThrPES = %e,\n Threshold for Hamiltonian "
           "matrix elements ThrMat = %e \n",
           ThrPES, ThrMat);
  } else {
    printf("\n Threshold for PES and for Hamiltonian matrix elements have to "
           "be > 0 \n");
    FinalMessaj() return 0;
  }
  if (NTargetStates) {
    if ((ThrCoor >= 1) || (ThrCoor < 0)) {
      printf("\n ***Threshold for coordinate must be lower than one and "
             "positive*** \n");
      FinalMessaj() return 0;
    } else {
...
per non converged eigenpairs at each iteration, bounded by %d \n",
  ...
  printf("\nMaximal allowed harmonic frequency in B+Bs Freq0Max = %.1f cm-1\n",
         Freq0Max);
  if (!DoGraph) {
    printf("\nDoGraph Null means no storage of residual graph: more cpu time, "
           "less memory\n");
    KNZREZ = 0;
  } else {
    printf("\nDoGraph non null means storage of residual graph: more memory, "
           "less cpu time\n");
  }
  if (EvalDeltaE) {
    printf("\nEvalDeltaE non null correction of order 2 will be printed out at "
           "final stage\n");
  }
 ...
 and shrinking factor for the maximal number of indexes in the residual graph KNZREZ =  %.1f\n",
  ...
  if (Memory <= 0) {
    printf(
        "****Need to indicate a positive value of Memory in megabytes****\n\n");
    FinalMessaj() return 0;
  }
  ...
  char BasisOut[30] = "-FinalBasis.bin"; // Final basis file name if PrintOut =
                                         // 1
  char VectOut[30] = "-Vectors.bin";     // Binary file name with targeted
                                         // eigenvectors if PrintOut = 1
  char MatOut[30] = "-Matrix.bin";       // Binary file name with targeted
                                         // eigenvectors if PrintOut > 1
  ...
  // Files to write Final basis set (FileBasis) and eigenvectors in binary
  // form(FileVec)
  if (PESType) // Derivatives in a.u
  {
    LinesKijCpld =
        GetNumKijCpld(FilePES, NMode, ThrPES, &NCPol, &DegrePol, DegreCoupl);
  } else // Force constants in cm-1
  {
    LinesKijCpld =
        GetNumKijCpld0(FilePES, NMode, ThrPES, &NCPol, &DegrePol, DegreCoupl);
  }
  ...
  printf("\nMax couplings NCPOL %i and max degree (sum of monomial indexes) "
         "DegrePol %i\n",
         NCPol, DegrePol);
  if (NCPol >= MaxCpld) {
  ...
  if (pow(NMode, 4) + LinesKijCpld > INT32_MAX) {
    printf("\nNumber of NNUL PES too big use int64_t to store them instead of "
           "int32_t\n\n");
    FinalMessaj() return 0;
  }
  ...
  allocate_MatInt(Monm, NPES, NMode)
      ...
      // Init Arrays
      InitMat(Monm, NPES, NMode) InitMat(KijNCpld, DegrePol, DegrePol * NMode)
          InitTabInt(KijCpld, NPES) double **CoorMode = NULL;
  allocate_MatDb(CoorMode, NMode, 3 * NAtome) double *CoorCart = NULL;
  ...
  int DoRotCheck =
      Min<int>(GetDataPES(FilePES, KijCpld, KijNCpld, Monm, CoorCart, Mas,
                          CoorMode, NMode, NAtome, PESType, Verbose, ThrPES),
               DoRot);
  if (!DoRotCheck) {
    DoRot = 0;
  }
  ...
  for (int mm = 0; mm < NMode; mm++) {
    if (PESType) {
      ...
      Pid[mm] = Min<uint32_t>(
          (uint32_t)ceil(Freq0Max / (pow(Omega[mm], 2) * HA_TO_CM)), MaxQLevel);
    } else {
      Omega[mm] = 2 * KijNCpld[1][mm];
      printf(" \\nu_{%d} : %.1f cm-1, ", mm + 1, Omega[mm]);
      Pid[mm] = Min<uint32_t>((uint32_t)ceil(Freq0Max / Omega[mm]), MaxQLevel);
      Omega[mm] =
          sqrt(Omega[mm] / HA_TO_CM); // Inverse conversion for Coriolis terms
    }
    if (((mm + 1) % 3 == 0) && (mm > 0)) {
      printf("\\\\\n");
    }
  ...
  double KFMax =
      AdjustCoeff(KijNCpld, KijCpld, DegrePol, NPES, PESType, Monm, NMode);
  printf("\nMaximal Force constant %f (exclude Harmonic)\n", KFMax);
  if ((ThrKX > 0) && (ThrKX < KFMax)) {
    printf("\n Threshold for acceptable excitations in H* : ThrKX = %e ,\n",
           ThrKX);
  } else {
    printf("\n ***Threshold for H* must be lower than KFMax and strictly "
           "positif*** \n");
    FinalMessaj() return 0;
  }
  // Force constants:
  // KFC. KijNCpld[dd][mm] : non coupled force constants, defined for (dd,mm) in
  // [0,DegrePol[x[0,NMode[ (degree dd+1, mode mm<NMode). KFC.KijCpld[kk] :
  // coupled force constants, defined for kk in [0,NPES[. KFC.Monm[kk][mm] =
  // degree of monomial kk for coordinate mm in PES.
  KForce KFC;
  allocate_MatDb(KFC.KijNCpld, DegrePol, NMode)
      allocate_TabDb(KFC.KijCpld, NPES) allocate_MatInt(KFC.Monm, NPES, NMode)
          InitKforce(KFC, KijNCpld, KijCpld, Monm, DegrePol, NMode,
                     NPES); // Gather Force constant data's in KFC structure.
  FreeMat(Monm, NPES) FreeTab(KijCpld) FreeMat(KijNCpld, DegrePol)
  ...
  printf("%ib_{%i} <= %i\n", (int)round(KFC.KijNCpld[1][NMode - 1] / FreqMin),
         NMode, (int)round(Freq0Max / FreqMin));
  ...
  if (DoRot) {
    ...
    Ull = MomentInertie(Mas, CoorCart, NAtome, MassC, PrincAx,
                        PrincEig); // Watson Correction
  ...
  //\mu_{\alpha,\beta}\zeta_{ij}^{\alpha}\zeta_{kl}^{\beta} , i<j, k<l
  double **ZetaXYZ = NULL;
  allocate_MatDb(ZetaXYZ, ((NMode + 1) * NMode) / 2,
                 ((NMode + 1) * NMode) / 2) if (DoRot) {
    if (Zeta(CoorMode, NMode, NAtome, Mas, PrincAx, PrincEig, ZetaXYZ, DoRot) <=
        0) {
      FinalMessaj();
      return 0;
    }
  ...
  uint32_t NGenPoz =
      NMode * DegreCoupl[0] + 1; // One Modal excitation+Zero excitation
  printf("\nMaximal degree for 1-mode couplings = %d\n", DegreCoupl[0]);
  for (int nn = 2; nn <= MaxCpld; nn++) {
    if (DoRot && nn <= 4) // Rotational terms may increase the couplings, except
                          // for water...
    {
      if (NMode > 3) {
  ...
  // Allocation for generator XPolSupPos[Idm(xx)+mm], (xx,mm) in [0,NGenPoz[ x
  // [0,NMode[
  allocate_TabMode(XPolSupPos,
                   NGenPoz) // Set of upper limit of positive excitations in H*
      uint32_t SizeGenOld =
          GeneratorTotPoz(NMode, XPolSupPos, DegreCoupl, NCPol);
  if (NGenPoz != SizeGenOld) {
    printf("**Modal Excitations not equal between each other SizeGenOld %d, "
           "NGenPoz %u**\n\n",
           SizeGenOld, NGenPoz);
    FinalMessaj() return 0;
  }
  ...
  InitTab(NLFFPerEx, NGenPoz) uint32_t NXDualHPlus =
      CptLFFPerEx(DegrePol, NMode, NPES, DoRot, NGenPoz, KFC, XPolSupPos,
                  NLFFPerEx, ZetaXYZ, ThrPES, IncK2);
  if (!NLFFPerEx[0]) {
    NXDualHPlus++;
  } // If zero excitation not included
...
(ni,nj,nk,nl) LFF.Idx[ii] >= NPES are key numbers of the rotational
coefficients nl*NMode^3+nk*NMode^2+nj*NMode+ni+NPES with a unique
corresponding (ni,nj,nk,nl)
*/
  NXDualHPlus = 0;
  uint32_t TotalLFF =
      0; // Total number of force constants per excitation in dual operator
  ...
  uint32_t AvgOp =
      (uint32_t)log((double)(NXDualHPlus)) * (double)NMode + AvgTerm * NMode;
  uint32_t MaxOp =
      (uint32_t)log((double)(NXDualHPlus)) * (double)NMode + MaxTerm * NMode;
  if (Verbose) {
    printf("\nAverage number of terms in the local force fields, AvgTerm = %u "
           "anf total %u \n",
           AvgTerm, TotalLFF);
    printf("\nMaximal number of terms in the local force fields (exclude 0), "
           "MaxTerm = %u\n",
           MaxTerm);
  ...
  uint32_t NXDualHTruncPos =
      AssignLFFPerEx(DegrePol, NMode, DoRot, NPES, NXDualHPlus, KFC, DualHPos,
                     LFF, ZetaXYZ, ThrKX, ThrPES, PermutGen, Corres, IncK2);
  if (Verbose) {
    printf("\nTotal number of raising excitations in H*\n(after truncation "
           "through ThrKX) NXDualHTruncPos-1 = %u\n",
           NXDualHTruncPos - 1);
  }
  uint32_t NXDualHTrunc =
      GetNXDualHTrunc(DualHPos, NXDualHTruncPos, Corres, NMode);
  if (Verbose) {
    printf("\nMaximal number of excitations in H*\n(after truncation through "
           "ThrKX) NXDualHTrunc = %u \n",
           NXDualHTrunc);
  }
  delete[] NLFFPerEx;
  delete[] XPolSupPos;
...
*/
  MatricesElem(QQ, DegrePol, MaxQLevel, DegrePol + 1 + 4)
      /*
      QQ[dd].Coeff[ii][jj] are matrix elements of operator Q^ii for ii in
      [0,DegrePol], QQ[DegrePol+1].Coeff[ii][jj] are matrix elements of operator
      D2Q (second order derivative) QQ[DegrePol+2].Coeff[ii][jj] are matrix
      elements of operator D1Q (first order derivative)
      ...
      */
      // Derivative terms
      for (int mm = 0; mm < 4; mm++){allocate_MatDb(
          QQ[mm + DegrePol + 1].Coeff, MaxQLevel + DegrePol,
          MaxQLevel + DegrePol)} MatrixD2Q(QQ[DegrePol + 1].Coeff,
                                           MaxQLevel + DegrePol)
  ...
  InitTabInt(ZeroSt, NMode) double FMax = 0;
  ...
  if ((MinFreq > 0) && (GroundState <= 0)) {
    printf("\n***GroundState value must be specified for window target when "
           "MinFreq>0***\n\n");
    FinalMessaj() return 0;
  }
  if ((MinFreq > 0) && (GroundState <= 0)) {
  ...
  // When the ground state is given <=0 it means that we actally want to compute
  // it.
  printf("\nMaximum targeted frequency %.1f, elongation factor Kappa %.1f \n",
         MaxFreq, Kappa);
  int SumTarget = 0;
  if (NTargetStates) {
    SumTarget = NTargetStates + AddTarget;
  } // Add target to allocate proper memory
  SizeActMax = GetSizeActMax2(
      MaxEv, MAXNCV, NMode, MaxQLevel, DegrePol, Verbose, NPES,
      NXDualHTruncPos - 1, Memory, KNNZ, KNREZ, KNZREZ, SumTarget, TotalLFF,
      NXDualHTrunc); // Compute maximal size from parameters
  ...
  allocate_TabMode(ModeAct, SizeActMax)
      // ModeAct : Multi-dimensional array for active space B :
      // ModeAct[Idm(nn)+mm], (nn,ii) in [0,SizeActMax[ x [0,NMode[. Changed to
      // ModeAct[Idm(nn)+mm]
      double TargetFreq =
          Kappa *
          MaxFreq; // Target frequency for initial subspace construction.
...
to avoid null intersection with targets\n",
  ...
  if (NTargetStates) {
    for (int tt = 0; tt < NTargetStates; tt++) {
      detect = LinearModeSearch(ModeAct, TargetState[tt],
                                NMode * sizeof(uint8_t), SizeInit);
      if (detect >= 0) {
    ...
    printf("\n");
    // Check zeros state
    detect = LinearU8Search(TargetState, ZeroSt, NMode * sizeof(uint8_t),
                            NTargetStates);
    if (detect < 0 && GroundState <= 0) {
      printf("\n**Ground state value must be provided when not included in "
             "targets **\n\n");
      FinalMessaj() return 0;
    } else if (detect >= 0 && GroundState > 0) {
      printf("\n**Ground state cannot be among targets when GroundState > "
             "0**\n\n");
      FinalMessaj() return 0;
    }
    // Check for doublons
      ...
      for (int jj = 0; jj < NTargetStates; jj++) {
        if (ii != jj) {
          if (PositionTarget[ii] == PositionTarget[jj]) {
            printf("Target %i and Target %i equal -> not possible all targets "
                   "must be differents \n\n",
                   ii, jj);
            FinalMessaj() return 0;
          }
  ...
  const uint64_t SizeRezMax =
      (uint64_t)((double)(SizeActMax * (NXDualHTruncPos - 1)) * KNREZ);
  ...
  const uint64_t NNZActMax =
      (uint64_t)((double)(SizeActMax * NXDualHTrunc) * (KNNZ));
  const uint64_t NNZRezMax =
      (uint64_t)((double)(SizeActMax * NXDualHTrunc) * (KNZREZ));
  printf("\nMaximal residual space size\n SizeRezMax "
         "(=SizeActMax*(NXDualHTruncPos-1)*KNREZ) : %lu \n",
         SizeRezMax);
  if (SizeRezMax > UINT32_MAX && DoGraph) {
    printf("\nMaximal residual space size too big for construction of the "
           "graph of\n");
    printf(" residual matrix, set DoGraph to 0 or decrease KNRez \n\n");
    FinalMessaj() return 0;
  }
  printf("\nMaximal NNZ for matrix Hb,\n NNZActMax "
         "(=SizeActMax*(KNNZ)*(NXDualHTrunc)) : %lu \n",
         NNZActMax);
  printf("Maximal indexes in graph of residual matrix Hsb,\n NNZRezMax (= "
         "SizeActMax*(KNZREZ)*(NXDualHTrunc)) : %lu \n\n",
         NNZRezMax);
  // ModeRez : Multi-dimensional array for residual space : ModeRez[Idm(nn)+mm],
  // (nn,mm) in [0,SizeRezMax[ x [0,NMode[. Changed to ModeRez[Idm(nn)+mm]
  allocate_TabMode(ModeRez, SizeRezMax)
      ...
      AllocateCSC(
          IJAct, NNZActMax,
          SizeActMax) // IJAct.NJ[jj],IJ.I[nn] in [0,SizeActMax[ x [0,NNZActMax[
      AllocateCSC(IJRez, NNZRezMax,
                  SizeActMax) // IJRez.NJ[jj],IJRez.I[nn] in
                              // [0,SizeRezMax[ x [0,NNZRezMax[
  ...
  double Ground =
      GroundState; // Ground state may be variable if we actually compute it
  int Iteration = 0;
  ...
  if (!MAXNCV) {
    MAXNCV = (NEV)*2;
  } // Automatic choice of number of Lanczos basis vectors : 2*NEV.
  ...
  InitTab(workd, 3 * SizeMax.DimAct + 1) double *workl = NULL;
  workl = new double[(MAXNCV) * (MAXNCV + 8) + 1];
  InitTabInt(workl, (MAXNCV) * (MAXNCV + 8) + 1) double *EigVec = NULL;
  EigVec = new double[SizeMax.DimAct * NCV + 1];
  InitTab(EigVec, SizeMax.DimAct * NCV + 1)
      // Component jj of Eigenvector ii is accessible at
      // EigVecii[ii*Size[Iteration+1].DimAct + jj]
  ...
  uint32_t *AssignRez = new uint32_t[MaxAdd]; // Store the indexes of residual
                                              // vectors selected components
  double DeltaFreq =
      0; // Maximal difference between previous and current eigenvalues
  ...
  for (int nn = 0; nn < NEV; nn++) {
    ...
    for (int mm = 0; mm < NMode; mm++) {
      ...
      EigValOld[nn] =
          EigValOld[nn] + (ModeAct[Idm(nn) + mm] + 0.5) * KFC.KijNCpld[1][mm];
    }
  }
 ...
 Maximal variation of eigenvalues DeltNEV = %i, \nFrequency window : [%f , %f] .\n",
  ...
  InitTab(RezVect, SizeMax.DimRez * NScreen)
      // Component jj of RezVect ii is accessible at RezVect[ii*SizeMax.DimRez +
      // jj]
      size_t SizeBit = NMode * sizeof(uint8_t); // sizeof(uint8_t)=1!
  int ExitSuccess = 1;
  int MaxAddVar =
      0; // MaxAddVar=Min<int>(MaxAdd,SizeActMax-Size[Iteration+1].DimAct);
  long DetectZero = 0; // If 1 the ground state has been detected
  long PosZero = -1;   // If > 0 give the position of the groundstate that might
                       // be non equal to zero
  while (MaxRelRez > EpsRez) // Main loop
  {
    ...
    CheckOutputAssemble = AssembleHarmCSC(
        NMode, DegrePol, NCPol, Iteration, NPES, ModeAct, Size, KFC, ValAct,
        IJAct, QQ, DegreCoupl, ZetaXYZ, Omega, ThrMat, NNZActMax, DualHPos, LFF,
    ...
    gettimeofday(&begin, NULL);
    // Set maximal number of Lanczos basis vectors for diagonalization in
    // eigensolver
    ...
    nconv = SolverCSCSym(Size, Iteration, NEV, NCV, Shift, Tol, EigVal, EigVec,
                         workd, workl, IJAct, ValAct);
    if (!nconv) {
    ...
    DeltaFreq = 0;
    // Get the varation of eigenvalues between current and last iteration
    // DeltaFreq=Max_l|E_l^(j)-E_l^(j-1)|
    for (int cc = 0; cc < nconv; cc++) {
      if (SIGN<double>(EigVal[cc] - EigValOld[cc]) *
              (EigVal[cc] - EigValOld[cc]) >
          DeltaFreq) {
        DeltaFreq = SIGN<double>(EigVal[cc] - EigValOld[cc]) *
                    (EigVal[cc] - EigValOld[cc]);
      }
    ...
    // Next iteration, the active space B has got minimum
    // Size[Iteration+1].DimAct elements Size[Iteration+2].DimAct will be
    // incremented for next iteration when adding new basis functions.
    ...
    PosZero =
        -1; // Position of groundstate if detected (might be not equal to zero)
    for (int PosEig = 0; PosEig < nconv; PosEig++) {
      // Sparse matrix-vector product HR*X
      // X eigenvector PosEig stored in
      // &EigVec[PosEig*Size[Iteration+1].DimAct], HR residual matrix
      if (GroundState <= 0) {
        DetectZero =
            DetectMaxCoordinate(&EigVec[PosEig * Size[Iteration + 1].DimAct],
                                PositionTarget, 1, ThrCoor);
        if (DetectZero) {
          PosZero = PosEig;
      ...
      if (NTargetStates && !DetectZero) {
        detect =
            DetectMaxCoordinate(&EigVec[PosEig * Size[Iteration + 1].DimAct],
                                PositionTarget, NTargetStates, ThrCoor);
      }
      // Freq[PosEig]=EigVal[PosEig]-Ground should be in the interval of
      // interest [MinFreq,MaxFreq+DeltaFreq]
      if (((EigVal[PosEig] - Ground <= MaxFreq + DeltaFreq) &&
           (EigVal[PosEig] - Ground >= MinFreq)) &&
          (detect || DetectZero)) {
        if (PosEig >= MaxScreen) {
          MaxScreen = PosEig;
        }
        ...
        NScreen++;
        if (Verbose) {
          // Print eigenvalue corresponding frequency, residues and ground state
          // for PosEig=0
          if (PosEig == PosZero) {
            printf("   %d   |   %.2f   |  %.2f  | ", PosEig, EigVal[PosEig],
                   EigVal[PosEig]);
          } else {
            printf("   %d   |  %.2f  |  %.2f  | ", PosEig, EigVal[PosEig],
                   EigVal[PosEig] - Ground);
          }
          // Assignment display
          AfficheNuSup(ModeAct, &EigVec[PosEig * Size[Iteration + 1].DimAct],
                       NMode, Size[Iteration + 1].DimAct, ThrCoor);
          printf(" || \n \n");
        }
      }
    }
    ...
    if (!NScreen) {
      printf("\nNot enough eigenvalues to reach targets -> increase MaxEV or "
             "DeltaNev,\n");
      printf("or maximal components are too small -> decrease ThrCoor,\n");
      printf("or initial subspace too small -> increase Kappa,\n");
      printf("or could also be the potential badly approximated -> decrease "
             "MaxQLevel.\n\n");
      FinalMessaj() ExitSuccess = 0;
      break;
    }
    if (NTargetStates && (NScreen > AddTarget + NTargetStates)) {
      ...
      printf("\nNot enough targets screened -> decrease ThrCoor\n or increase "
             "MaxEv, DeltaNev and/or Kappa\n\n");
      FinalMessaj() ExitSuccess = 0;
      break;
    }
    gettimeofday(&begin, NULL);
    if (DoGraph) { // The space H*(A) is fetched at each iteration where A is
                   // the set of added basis functions.
      CheckOutputPolyX =
          PolyX(ModeRez, ModeAct, DualHPos, QQ, PermutRez, PermutAct, Corres,
                KFC, Size, Pid, Freq0Max, TargetMax, NMode, NXDualHTruncPos,
                NCPol, DegrePol, NPES, Iteration, NScreen, TabScreen, SizeMax,
                RezVect, LFF, EigVec, ZetaXYZ, Omega, IJRez);
    } else { // The whole space H*(B) is fetched at each iteration.
      CheckOutputPolyX =
          PolyXNoG(ModeRez, ModeAct, DualHPos, QQ, PermutRez, PermutAct, Corres,
                   KFC, Size, Pid, Freq0Max, TargetMax, NMode, NXDualHTruncPos,
                   NCPol, DegrePol, NPES, Iteration, NScreen, TabScreen,
                   SizeMax, RezVect, LFF, EigVec, ZetaXYZ, Omega);
    }
    if (CheckOutputPolyX == 0) {
      printf("\n|B|: %u , |Bs|: %lu, \n NNZ(Hb) : %lu , NNZ(Hsb) : %lu \n",
             Size[Iteration + 1].DimAct, Size[Iteration + 1].DimRez,
             Size[Iteration + 1].NNZAct, Size[Iteration + 1].NNZRez);
      MaxRelRez = 0;
      ExitSuccess = 0;
      break;
    }
    ...
    if (Iteration && DoGraph) {
      gettimeofday(&begin, NULL);
      // Matrix vector product of residual matrix on the fly completed thanks to
      // graph stored in IJRez
      FlyRezMVP2(ModeRez, ModeAct, DualHPos, QQ, PermutGen, TabNull, KFC, Size,
                 NMode, IJRez, NXDualHPlus, DegrePol, NPES, Iteration, NScreen,
                 TabScreen, SizeMax, RezVect, LFF, EigVec, ZetaXYZ, Omega);
      gettimeofday(&end, NULL);
      time_spent = TIME_DIFFS(begin, end);
      if (Verbose) {
    ...
    for (int ll = 0; ll < NScreen; ll++) {
      RezRel[ll] =
          Norm2F(&RezVect[SizeRezMax * ll], Size[Iteration + 1].DimRez) /
          EigVal[TabScreen[ll]];
      if (TabScreen[ll] == PosZero) // Last position of zero readed
      {
        printf("   %d   |  %.4f  |  %.4f  | ", TabScreen[ll],
               EigVal[TabScreen[ll]], RezRel[ll]);
      } else {
        printf("   %d   |  %.4f  |  %.4f  | ", TabScreen[ll],
               EigVal[TabScreen[ll]] - Ground, RezRel[ll]);
      }
      // Assignment display
      AfficheNuSup(ModeAct, &EigVec[TabScreen[ll] * Size[Iteration + 1].DimAct],
                   NMode, Size[Iteration + 1].DimAct, ThrCoor);
      printf(" || \n \n");
    ...
    if (MaxRelRez > EpsRez) {
      float MaxComp = 0; // Sum of maximal component of maximal component of
                         // residual vectors
      int NNotConv = 0; // number of non converged eigenvector at this iteration
      for (int ll = 0; ll < NScreen; ll++) {
        if (RezRel[ll] > EpsRez) {
          MaxComp += FindMaximumValPoz<float>(&RezVect[SizeRezMax * ll],
                                              Size[Iteration + 1].DimRez);
          TabScreen[NNotConv] = ll;
          NNotConv++;
        }
      }
      // The entries of the RezVec greater than to ||R||/(EtaComp*NNotConv) will
      // be considered for enrichment
      MaxComp /= (EtaComp * NNotConv);
      if (Verbose) {
        printf("Maximal component of error vector divided by EtaComp*NNotConv "
               "%f \n",
               MaxComp);
      }
      ...
      if (MaxAddVar) {
        ...
        NAddVar = MaxContribAll(RezVect, AssignRez, TabScreen, TabNull, DoGraph,
                                Size[Iteration + 1].DimRez, NAddIter, MaxAddVar,
                                MaxComp, NNotConv, NScreenTot, SizeRezMax);
        gettimeofday(&end, NULL);
        time_spent = TIME_DIFFS(begin, end);
        printf("Number of added basis functions |A| : %i, Time spent to get "
               "them : %u s\n",
               NAddVar, time_spent);
      } else {
        printf("\n|B|: %u , |Bs|: %lu, \n NNZ(Hb) : %lu , NNZ(Hsb) : %lu \n",
               ...
               Size[Iteration + 1].NNZAct, Size[Iteration + 1].NNZRez);
        printf(
            "Maximum number of basis functions reached at previous iteration\n\
-> Algorithm stop at iteration %i \n",
            Iteration);
        MaxRelRez = 0;
        break;
      }
      for (int aa = 0; aa < NAddVar;
           aa++) { // Add elements for next iteration and increment
                   // Size[Iteration+2].DimAct
        if (Size[Iteration + 2].DimAct < SizeActMax) {
          memcpy(&ModeAct[Idm(Size[Iteration + 2].DimAct)],
                 &ModeRez[Idm(AssignRez[aa])], SizeBit);
          Size[Iteration + 2].DimAct++;
        }
      }
    } else {
    ...
    if (Iteration >= IterMax - 1) {
      printf("Maximal number of iteration reached at iteration %i -> algorithm "
             "stop\n \n",
             Iteration);
      MaxRelRez = 0;
      break;
  ...
  if (ExitSuccess) {
    ...
    if (EvalDeltaE) {
      ...
      InitTabInt(VPTE, NScreen)
          CorrectionEnergy(RezVect, Size[Iteration + 1].DimRez, EigVal, ModeRez,
                           NScreen, NPES, DegrePol, NMode, TabScreen, QQ,
                           SizeMax, LFF, ZetaXYZ, Omega, VPTE, KFC);
      gettimeofday(&end, NULL);
      time_spent = TIME_DIFFS(begin, end);
      // Define the maximum size of active space B and residual space
    ...
    if (EvalDeltaE) {
      ...
      printf(
          " (Component) | (number)  | Residue |   Energy   | Ref-Here || \n");
      printf("------------------------------------ \n");
    } else {
    ...
    for (int PosEig = 0; PosEig < nconv; PosEig++) {
      DetectZero = 0; // If >0 the groundstate has been detected
      detect = 1; // If it remains positive one of the Target has been detected
      PosZero = -1; // Position of groundstate if detected (might be not equal
                    // to zero)
      if (GroundState <= 0) {
        DetectZero =
            DetectMaxCoordinate(&EigVec[PosEig * Size[Iteration + 1].DimAct],
      ...
      if (((EigVal[PosEig] - Ground <= MaxFreq + DeltaFreq) &&
          ...
          (detect || DetectZero)) {
        // Assignment display
        AfficheNuSupTex(ModeAct, &EigVec[PosEig * Size[Iteration + 1].DimAct],
                        NMode, Size[Iteration + 1].DimAct, ThrCoor);
        if (PosEig == PosZero) {
          if (EvalDeltaE) {
            if (!DoRot) {
              printf(" | %.2f(%d) | %.4f | %.4f  | %.4f  |  // \n \n", Ground,
                     PosEig, RezRel[ss], VPTE[ss],
                     GetClosest(TabRef, Ground) - (Ground));
            } else // Add Watson
            {
              printf(" | %.2f(%d) (mu : %.2f) | %.4f | %.4f | %.4f  // \n \n",
                     Ground, PosEig, Ull, RezRel[ss], VPTE[ss],
                     GetClosest(TabRef, Ground) - (Ground)); // Add watson
            }
          } else {
            if (!DoRot) {
              printf(" | %.2f(%d) | %.4f | %.4f // \n \n", Ground, PosEig,
                     RezRel[ss], GetClosest(TabRef, Ground) - (Ground));
            } else // Add Watson
            {
              printf(" | %.2f(%d) (mu : %.2f) | %.4f | %.4f  // \n \n", Ground,
                     PosEig, Ull, RezRel[ss],
                     GetClosest(TabRef, Ground) - (Ground)); // Add watson
            }
          }
        } else {
          if (EvalDeltaE) {
            ...
            printf(" | %.2f(%d) | %.4f | %.4f | %.4f // \n \n",
                   EigVal[PosEig] - Ground, PosEig, RezRel[ss], VPTE[ss],
                   GetClosest(TabRef, EigVal[PosEig] - Ground) -
                       (EigVal[PosEig] - Ground));
          } else {
            printf(" | %.2f(%d) | %.4f | %.4f // \n \n",
                   EigVal[PosEig] - Ground, PosEig, RezRel[ss],
                   GetClosest(TabRef, EigVal[PosEig] - Ground) -
                       (EigVal[PosEig] - Ground));
          }
        }
    ...
    {
      PrintConfsBin(OutBasis, ModeAct, NMode, NScreen,
                    Size[Iteration + 1].DimAct);
      PrintVecBin(OutVec, EigVec, EigVal, NScreen, TabScreen, SaveZero,
                  GroundState, Size[Iteration + 1].DimAct);
    }
    if (PrintOut > 1) // Matrix in binary
    {
      PrintMatCSCBin(OutMat, Size, Iteration, NEV, NCV, SizeInit, Shift, Tol,
                     Ull, IJAct, ValAct);
      PrintConfsBin(OutBasis, ModeAct, NMode, NScreen,
                    Size[Iteration + 1].DimAct);
    }
           ...
           PrintOut) // Indicate non success with null value into binarry files
  {
  ...
  delete[] ModeRez;
  delete[] DualHPos;
  FreeMat(ZetaXYZ, ((NMode + 1) * NMode) / 2) FreeMat(KFC.KijNCpld, DegrePol)
      FreeMat(KFC.Monm, NPES) FreeMat(CoorMode, NMode) delete[] KFC.KijCpld;
  FreeCSC(IJAct) FreeCSC(IJRez) FreeLFF(LFF) delete[] Size;
  delete[] RezRel;
  delete[] Omega;
  delete[] PermutGen;
  delete[] Mas;
