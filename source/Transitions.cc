...
https://doi.org/10.1016/j.cpc.2018.07.008\n");
  ...
  double MaxFreq = 4000;     double GroundState = -1; // Groundstate should be given by the user.
...
per non converged eigenpairs at each iteration, bounded by %d \n",
 ...
 and shrinking factor for the maximal number of indexes in the residual graph KNZREZ =  %.1f\n",
  ...
  if (!PrintOut) {
    printf("\n***PrintOut must be equal to 1 and basis set saved from previous "
           "DVCI run*** \n\n");
    FinalMessaj() return 0;
  }
  ...
  FreeMat(Monm, NPES) FreeTab(KijCpld)
      FreeMat(KijNCpld, DegrePol) if (PrintOut > 2) {
    ...
    for (int NEccit = 1; NEccit <= NMode; NEccit++) {
      ...
      do {
        ...
        if (AdjustFreq >= 0) {
          for (int mm = 0; mm < NMode; mm++) {
            if (MaxDegSurf[mm]) {
              // ceil(Freq0Max-SubFreq+KFC.KijNCpld[1][mm])/KFC.KijNCpld[1][mm]
              // because starts at 1 and strictly inferior : Maxn=Maxn+1
              MaxDegrees[ss] = Min<int>(
                  (int)ceil((AdjustFreq) / KFC.KijNCpld[1][mm]) + 1, MaxQLevel);
              MultiDegrees[ss] = 1;
              MultiSurf[ss] = mm;
              ss++;
            }
                    ...
                    // printf("\n");
          do {
            // Sum_ee Multidegrees[ee] is all the possible sum of NEccit uplets
            // lower than MaxDegrees+1.
            ffreq = 0;
  ...
  if (DoRot) {
    ...
    double Ull = MomentInertie(Mas, CoorCart, NAtome, MassC, PrincAx,
  ...
  /*LFF[xx] : local force field associated with positive excitation
  DualHPos[xx]. LFF[xx].Idx[ii] < 0 are indexes of non coupled force constants:
  KFC.KijNCpld[dd][mm] with dd=-LFF.Idx[ii]/NMode, mm=-LFF.Idx[ii]-dd*NMode;
  0 <= LFF[xx].Idx[ii] < NPES and are indexes of coupled force constants
  KFC.KijCpld[LFF[xx].Idx[ii]]. LFF.Idx[ii] >= NPES are key numbers of the
  rotational coefficients nl*NMode^3+nk*NMode^2+nj*NMode+ni+NPES with a unique
  corresponding (ni,nj,nk,nl) LFF[xx].Idx[ii] are defined for ii in
  [0,LFF[xx].Num[
  ...
  */
  ...
  double TargetFreq =
      Kappa * MaxFreq; // Target frequency for initial subspace construction.
...
to avoid null intersection with targets\n",
  ...
  // form(FileVec) Take the previous name generated by DVCI
  ...
  if (NTargetStates) {
    for (int tt = 0; tt < NTargetStates; tt++) {
      detect = LinearModeSearch(ModeAct, TargetState[tt],
                                NMode * sizeof(uint8_t), FinalSize);
      if (detect >= 0) {
  ...
  AllocateCSC(
      IJAct, NNZActMax,
      FinalSize) // IJAct.NJ[jj],IJ.I[nn] in [0,FinalSize[ x [0,NNZActMax[
      //    CSC values
      double *ValAct = NULL;
  ...
  InitTab(ValAct, NNZActMax)
      ...
      uint32_t CheckOutputAssemble =
          AssembleHarmCSC(NMode, DegrePol, NCPol, 0, NPES, ModeAct, Size, KFC,
                          ValAct, IJAct, QQ, DegreCoupl, ZetaXYZ, Omega, ThrMat,
                          NNZActMax, DualHPos, LFF, PermutGen, NXDualHPlus,
                          IncK2);
    if (!CheckOutputAssemble) {
    printf("Faile assemble matrix operator\n");
    FinalMessaj() return 0;
  }
  // Read Vectors from binary file and corresponding eigenvalue in position
  // Size[1].DimAct*ii
  double *EigVec = NULL;
  EigVec = new double[(Size[1].DimAct + 1) * NScreen];
  for (int ii = 0; ii < NScreen; ii++) {
    fread(&EigVec[ii * (Size[1].DimAct + 1)], sizeof(double),
          Size[1].DimAct + 1, FileVec);
  }
  ...
  for (int ii = 0; ii < NScreen; ii++) {
    double Trans =
        ScualProd(&EigVec[ii * (Size[1].DimAct + 1)], Y, Size[1].DimAct);
    printf("< Phi_0 | %s |  Phi_%d > : %f ,\n", DipoleChar, PositionTarget[ii],
           Trans);
    printf("Corresponding energy transition E_%d - E_0 : %f\n\n",
           PositionTarget[ii], EigVec[(ii + 1) * (Size[1].DimAct) + ii] - E0);
  }
  ...
  FreeMat(ZetaXYZ, ((NMode + 1) * NMode) / 2)
            FreeMat(KFC.KijNCpld, DegrePol) FreeMat(KFC.Monm, NPES)
          FreeMat(CoorMode, NMode) delete[] KFC.KijCpld;
    FreeCSC(IJAct) FreeLFF(LFF) delete[] Size;
  delete[] Omega;
  delete[] PermutGen;
  delete[] Mas;
  delete[] CoorCart;
