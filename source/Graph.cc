...
int nested_loop(uint8_t *ix, uint8_t *im, int depth) {
  // Simulation of a nested loop of a given depth to generate multi-indexes
  // stored in ix with ix[Cpt] bounded by im[Cpt]. ix must be initialized by 0.
  int Cpt = 0;
  do {
    ix[Cpt]++;
    if (ix[Cpt] < im[Cpt]) {
...
long LinearModeSearch(uint8_t *TabMode, uint8_t *ModeT, size_t NMode,
  ...
  // TabMode. Size of TabMode is SizeTab*SizeBit. Don't forget the case where the
  // size of array is zero. size_t SizeBit=NMode*sizeof(uint8_t)=NMode;
  uint32_t Cmpt = 0;
  // memcmp return 0 if the configurations are identical else it returns a non
  // nul value.
...
long QsearchModeShift(uint8_t *Key, uint8_t *base, uint32_t *Permuter,
  ...
  // Same as previous subroutine : Binary search of configurations "Key" in
  // basis "base" already sorted in the memcmp order given by the indexes of
  // Permuter array. But indexes are shifted by the number Shift in order to
  // avoid troubles when they oversize MAXUINT32 Because Permuter is an array of
  // UINT32_t
  uint32_t debut = 0, fin = size - 1;
  uint32_t milieu = 0;
  int Cmp = 0;
...
int32_t partition(uint8_t *arr, uint32_t Permuter[], int ll, int rr,
                  int NMode) {
  /*
...
int32_t PartShift(uint8_t *arr, uint32_t Permuter[], int ll, int rr, int NMode,
                  uint64_t Shift) {
  /*
  ...
  than can be bigger than UINT32MAX. Comparison of elements is done with memcmp
  c subroutine.
  */
  int32_t ii = (ll - 1);

  for (int32_t jj = ll; jj <= rr - 1; jj++) {
        if (memcmp(&arr[Idm(Permuter[jj] + Shift)], &arr[Idm(Permuter[rr] + Shift)],
               NMode) <= 0) {
            ii++;
      swap(&Permuter[ii], &Permuter[jj]);
    }
...
void QuickSortMode(uint8_t *MIndex, int lo, int hi, unsigned int *Permuter,
                   int NMode) {
...
void QuickSortModeShift(uint8_t *MIndex, int lo, int hi, uint32_t *Permuter,
                        int NMode, uint64_t Shift) {
  // Sort multi-indexes in &MIndex[+Shift] according to memcmp order and return
  // ascending order indexes in Permuter
  if (lo < hi) {
...
uint32_t InitB0(uint8_t *ModeAct, KForce KFC, uint8_t *Pid, int NMode,
  ...
  // Here is used only one modal nested excitations on the ground state
  // (0,...,0). Pid[mm] is the maximal quantum level + 1 in each direction mm
  ...
  // degree of monomial kk for coordinate mm in PES. ModeAct : Multi-dimensional
  // array for active space B : ModeAct[Idm(nn)+mm], (nn,mm) in [0,SizeActMax[ x
  // [0,NMode[.
  uint32_t Lin = 0;
  ...
  int TestNeg =
      0; // Test if the sum is negative, here only check the max dimension
  int Cpld;
  double Freq0, Freq0T;
  uint32_t SizeAv = 0;
  uint32_t SizeAvAv = 0;
        ...
        if (go) {
          ...
          if (Freq0T <= TargetMax) {
            if (SizeInit < SizeActMax) {
              ...
              // configuration It has to be negative if not. Tester=0 and
              // Size0.DimAct-SizeAv : no problem because CheckIn >=0 Tester non
              // equal zero no problem either because not in previous checking.
              // Checking can be done starting from previous layer because
              // before the gap will be bigger than 1 degree difference
                            CheckIn = LinearModeSearch(&ModeAct[Idm(SizeAvAv)], Tester,
                                         SizeBit, SizeInit - SizeAvAv + 1);
              if (CheckIn < 0) {
                                memcpy(&ModeAct[Idm(SizeInit)], Tester, SizeBit);
                SizeInit++;
              }
            } else {
              printf("***Init subpsace oversized->Increase memory***\n");
              return 0;
            }
          }
        }
...
void VoisinCpldRot(int NMode, uint8_t *Tester, uint8_t *NVMode,
  ...
  Return neighboor-indexes of the normal coordinate mm available in the array
...
void AfficheNuTex(uint8_t *Mode,
                  int NMode) // Print Modal components in Tex format
...
void AfficheNuSupTex(
    uint8_t *ModeAct, double *EigVec, int NMode, uint32_t DimAct,
    double ThrCoor) { // Print Modal components in Tex format and vector
                      // components bigger than ThrCoor
  int Cpt = 0;
  double Val;
...
uint32_t CptLFFPerEx(int DegrePol, int NMode, int NPES, int DoRot,
                     uint32_t NGenPoz, KForce KFC, uint8_t *XPolSupPos,
  ...
  // (ni,nj,nk,nl) NLFFPerEx[xx] : output array counting the number of force
  // constant per excitation XPolSupPos[xx] ThrPES : threshold for acceptable
  // force constants. NPES : Number of coupled PES coefficients stored in
  // KFC.KijCpld ZetaXYZ[ijkl]=\sum_{(\alpha ,\beta)\in (x,y,z)}
  // \mu_{\alpha,\beta}\zeta_{ij}^{\alpha}\zeta_{kl}^{\beta} IncK2:Say if the the
  // quadratic term should be included or not in the local force field
  ...
  // Allocation
  allocate_MatUint8(ModeV, DegrePol + 1, NMode) allocate_TabUint8(NVMode, NMode)
      allocate_TabUint8(MultiDegrees, NMode) allocate_TabUint8(Tester, NMode)
      // Initialisation of the degrees of the maximal mode
      // int Cmpt=1; //First is zero degree
      int Deg = 0;
  double ContribLFF =
      0; // Contribution of force constants summed up in absolute value
  long IndexEx;
  ...
  for (int mm = 0; mm < NMode; mm++) {
    InitTabInt(Tester, NMode) for (int kk = 0; kk < DegrePol; kk++) {
  ...
  for (int kk = 0; kk < NPES; kk++) {
    ...
    if (ContribLFF > ThrPES) {
      ...
      InitTabInt(MultiDegrees, NMode) do {
        for (int mm = 0; mm < NMode; mm++) {
      ...
      for (int nj = ni + 1; nj < NMode; nj++) {
        ...
        for (int nk = 0; nk < NMode; nk++) {
          for (int nl = nk + 1; nl < NMode; nl++) {
            ...
            if (ContribLFF > ThrPES) {
              // Tester is the equivalent monomial c^(ijkl)= 1i+1j+1k+1l, (sum
              ...
              InitTabInt(MultiDegrees, NMode) do {
                ...
                IndexEx =
                    QsearchMode(Tester, XPolSupPos, Permuter, NGenPoz, SizeBit);
                NLFFPerEx[IndexEx]++;
              }
              while (nested_loop(MultiDegrees, NVMode, NMode))
                ;
  ...
  delete[] Permuter;
  delete[] Tester;
    FreeMat(ModeV, DegrePol + 1) return NXDualHPlus;
}
uint32_t AssignLFFPerEx(int DegrePol, int NMode, int DoRot, int NPES,
  ...
  // indexes of the first NXDualHTruncPos most contributive positive excitations
  // of H*. Select only the ones such that Sum|FC|>ThrKX DualHPos contains
  // positive excitations in H*. DualHPos[Idm(xx)+mm] is defined for (xx,mm) in
  // [0,NXDualHPlus[ x [0,NMode[. Force constants: KFC. KijNCpld[dd][mm] : non
  // coupled force constants, defined for (dd,mm) in [0,DegrePol[x[0,NMode[
  // (degree dd+1, mode mm<NMode). KFC.KijCpld[kk] : coupled force constants,
  // defined for kk in [0,NPES[. KFC.Monm[kk][mm] = degree of monomial kk for
  // coordinate mm in PES. Local force field: LFF[ii(xx)] : local force field
  // associated with positive excitation &DualHPos[Idm(xx)]. LFF.Idx[ii(xx)] are
  // defined for ii [LFF.Num[xx],LFF.Num[xx+1][ LFF.Idx[ii(xx)] < 0 are indexes
  // of non coupled force constants: KFC.KijNCpld[dd][mm] with
  ...
  // nl*NMode^3+nk*NMode^2+nj*NMode+ni+NPES with a unique corresponding
  // (ni,nj,nk,nl) Permuter : permutation index array of elements of DualHPos
  // (memcmp order) ThrPES : threshold for acceptable force constants. NPES :
  // Number of coupled PES coefficients stored in KFC.KijCpld
  // ZetaXYZ[ijkl]=\sum_{(\alpha ,\beta)\in (x,y,z)}
  // \mu_{\alpha,\beta}\zeta_{ij}^{\alpha}\zeta_{kl}^{\beta} NXDualHTruncPos :
  // output = Total number of raising excitations for H* (after truncation
  // through ThrKX) IncK2:Say if the the quadratic term should be included or not
  ...
  LFFNUM = new uint32_t[NXDualHPlus + 1];
    ...
    double
      ContribLFF; // Contribution of force constants summed up in absolute value
    ...
    InitTabInt(Tester, NMode) for (int kk = 0; kk < DegrePol; kk++) {
      ...
      if (ContribLFF > ThrPES) {
        ...
        while (Deg >= 0) {
          Tester[mm] = Deg;
          IndexEx =
              QsearchMode(Tester, DualHPos, Permuter, NXDualHPlus, SizeBit);
          if (IndexEx >= 0) {
            //       LFF[IndexEx].Idx[LFF[IndexEx].Num]=-((NMode)*(kk)+mm);
            LFF.Idx[LFFNUM[IndexEx]] = -((NMode) * (kk) + mm);
            //       LFF[IndexEx].Num++; Starting incremented up to
            //       LFF.Num[IndexEx+1];
            LFFNUM[IndexEx]++;
          ...
          for (int nl = nk + 1; nl < NMode; nl++) {
            ...
            if (ContribLFF > ThrPES) {
              ...
              InitTabInt(MultiDegrees, NMode) do {
                ...
                IndexEx = QsearchMode(Tester, DualHPos, Permuter, NXDualHPlus,
                                      SizeBit);
                if (IndexEx >= 0) {
                  //       LFF[IndexEx].Idx[LFF[IndexEx].Num]=NPES+NumRot;
                  //       LFF[IndexEx].Num++;
                  LFF.Idx[LFFNUM[IndexEx]] = NPES + NumRot;
                  LFFNUM[IndexEx]++; // Starting incremented up to
                                     // LFF.Num[IndexEx+1];
                  Contrib[IndexEx] += ContribLFF;
                }
              }
  ...
  delete[] Tester;
  delete[] Contrib;
  FreeMat(ModeV, DegrePol + 1) return NXDualHTruncPos;
}
