...
In routine MomentInertie, the computation of principal axes of inertia momentum
requires to diagonalize a matrix with DSYEV subroutine of LAPACK librairy[1].

[1] E. Anderson, Z. Bai, C. Bischof, S. Blackford, J. Demmel, J. Dongarra, J. Du
Croz, A. Greenbaum, S. Hammarling, A. McKenney, D. Sorensen, LAPACK Users’
Guide, 3rd Edition, Society for Industrial and Applied Mathematics,
...
The rotational coefficients are computed according to the method of Meal and
Polo [2]

[2]Vibration—Rotation Interaction in Polyatomic Molecules. II. The Determination
of Coriolis Coupling Coefficients Janet Hawkins Meal and S. R. Polo, The Journal
of Chemical Physics 1956 24:6, 1126-1133
...
#include <sys/time.h>
double
GetEHarmonic0(uint8_t *Harm, int NMode,
              KForce KFC) { // Return Harmonic energy sum_NM (d_i+0.5)*\nu_i
  double Val = 0;
...
double GetFreq0(uint8_t *Harm, int NMode,
                KForce KFC) { // Return Harmonic frequency sum_NM (d_i)*\nu_i
  double Val = 0;
  for (int mm = 0; mm < NMode; mm++) {
...
void InitKforce(KForce KFC, double **KijNCpld, double *KijCpld, int **Monm,
                int DegrePol, int NMode, int NPES) {
  CopyMat(KFC.KijNCpld, KijNCpld, DegrePol, NMode)
      CopyMat(KFC.Monm, Monm, NPES, NMode) CopyTab(KFC.KijCpld, KijCpld, NPES)
}
void DeleteKForce(KForce &KFC, int DegrePol, int NPES) {
  FreeMat(KFC.KijNCpld, DegrePol) FreeMat(KFC.Monm, NPES) FreeTab(KFC.KijCpld)
}
...
int DetectMaxCoordinate(double *Vec, int *PositionTarget, int NTargetStates,
                        double ThrCoor) {
  // Return 1 if a coordinate listed in PositionTarget is bigger than ThrCoor in
  // absolute value. Return 0 if no such coordinate have been detected when
  // NTargetStates>0 Return 1 when NTargetStates=0
...
uint32_t MaxContribAll(float *Vec, uint32_t *IndexAssign, int *TabScreen,
                       uint32_t *TabNull, int DoGraph, const uint64_t DimRez,
                       const int NAdd, int MaxAdd, const float Thr,
                       int NNotConv, int NScreenTot, uint32_t SizeRezMax) {
  ...
  // TabNull[nn] is equal to zero when residual contribution nn has been chosen
  // for enrichment in previous iteration (else 1) Vec[nn*SizeRezMax] contains
  // the eigenvector number nn. Nadd = number of elments to add at next
  // iteration per non converged eigenpairs NNotConv. MaxAdd = Limit of number
  // of elments to add at next iteration. Thr = Threshold above which a
  // coordinate will be selected for enrichment
  int NScreenNC = 0;
  ...
  while (cmpt < NNAdd && NScreenNC < MaxAdd) {
    ...
    while (ll < NNotConv && NScreenNC < MaxAdd) {
      IndexAssign[NScreenNC] =
          FindMaximumAbs<float>(&Vec[TabScreen[ll] * SizeRezMax], DimRez);
...
void MassCenter(double *Mas, double *CoorCart, int NA, double *MassC) {
  ...
  InitTab(MassC, 3) for (int jj = 0; jj < 3; jj++) {
...
double MomentInertie(double *Mas, double *CoorCart, int NA, double *MassC,
                     double *PrincAx, double *PrincEig) {
  ...
  // X_n =CoorCart[3*(n-1)], Y_n =CoorCart[3*(n-1)+1], Z_n =CoorCart[3*(n-1)+2],
  for (int ii = 0; ii < NA; ii++) {
    Ixx += Mas[ii] * (pow(CoorCart[3 * (ii) + 1] - MassC[1], 2) +
                      pow(CoorCart[3 * (ii) + 2] - MassC[2], 2));
    Iyy += Mas[ii] * (pow(CoorCart[3 * (ii)] - MassC[0], 2) +
                      pow(CoorCart[3 * (ii) + 2] - MassC[2], 2));
    Izz += Mas[ii] * (pow(CoorCart[3 * (ii)] - MassC[0], 2) +
                      pow(CoorCart[3 * (ii) + 1] - MassC[1], 2));
    Iyz -= Mas[ii] * ((CoorCart[3 * (ii) + 1] - MassC[1]) *
                      (CoorCart[3 * (ii) + 2] - MassC[2]));
    Ixz -= Mas[ii] * ((CoorCart[3 * (ii)] - MassC[0]) *
                      (CoorCart[3 * (ii) + 2] - MassC[2]));
    Ixy -= Mas[ii] * ((CoorCart[3 * (ii) + 1] - MassC[1]) *
                      (CoorCart[3 * (ii)] - MassC[0]));
  }
  ...
  dsyev_(&JOBZ, &UPLO, &Dim, &PrincAx[0], &Dim, &PrincEig[0], WORK, &LWORK,
         &INFO);
  // Tensor constant to shift the fundamental value by U=-1/8\sum _{\alpha
  // =1}^{3}\mu_{\alpha \alpha }
  return -(1.0 / 8.0) * (1 / PrincEig[0] + 1 / PrincEig[1] + 1 / PrincEig[2]) *
         HA_TO_CM;
}
int Zeta(double **CoorMode, int NMode, int NA, double *Mas, double *PrincAx,
         double *PrincEig, double **ZetaXYZ, int DoRot) {
  // Routines that compute the Zeta coefficients according to the method of Meal
  // and Polo PrincAxes are eigenvectors of inertia momentum matrix PrincEig are
  // eigenvalues of inertia momentum matrix Normal coordinate vectors are
  // CoorMode[ii][3*(jj)], CoorMode[ii][3*(jj)+1], CoorMode[ii][3*(jj)+2], 0 <=
  // jj< NA, 0<= ii <NMode
  // DoRot specifies non mass weighted normal coordinates for odd value and mass
  // weighted for even value
  double Ui[3] = {0};
  double Vj[3] = {0};
  double W[3] = {0};
  ...
  InitTabInt(ZetaX, NMode * NMode) InitTabInt(ZetaY, NMode * NMode)
      InitTabInt(ZetaZ, NMode * NMode) double LocalNormii = 0;
  double ScalTest = 0;
  for (int ii = 0; ii < NMode; ii++) {
    LocalNormii = 0;
    for (int aa = 0; aa < NA; aa++) {
      if (DoRot %
          2) // 2 possible type of normal coordinates : mass weighted or not.
      {
    ...
    if (fabs(LocalNormii - 1) > 1e-5) {
      printf(
          "Normal coordinates eigenvectors are not normal : change DoRot\n\n");
      return 0;
    }
  }
  for (int ii = 0; ii < NMode; ii++) {
  ...
  for (int ii = 0; ii < NMode; ii++) {
    for (int jj = ii + 1; jj < NMode; jj++) {
      for (int aa = 0; aa < NA; aa++) {
        // Mass weighted displacements vectors. Careful the transformation is
        // orthogonal Then the weighted vectors should have total norm equal to
        // one. Zeta matrix definite with mass weighted coordinates. We have the
        // GF transformation without mass weight. Then we have to scale the GF
        // matrix by [sqrt(M1)*I3,...,sqrt(MN)*I3]
    ...
    for (int jj = ii + 1; jj < NMode; jj++) {
      for (int kk = 0; kk < NMode; kk++) {
        for (int ll = kk + 1; ll < NMode; ll++) {
          ...
          InitTab(ZetaT1, 3) InitTab(ZetaT2, 3)
              // ZetaT1=P^T Zeta1
          ...
          //=zeta_ij^x zeta_kl^x RotC_x + zeta_ij^y zeta_kl^y RotC_y + zeta_ij^z
          // zeta_kl^z RotC_z
          ZetaXYZ[ii + ((jj - 1) * jj) / 2][kk + ((ll - 1) * ll) / 2] =
              ScualProd(ZetaT2, ZetaT1, 3) * HA_TO_CM;
          // ZetaXYZ[ijkl]=\sum_{(\alpha ,\beta)\in (x,y,z)}
          // \mu_{\alpha,\beta}\zeta_{ij}^{\alpha}\zeta_{kl}^{\beta}
        }
      }
    }
  }
...
uint32_t GetSizeActMax2(int MaxEv, int MAXNCV, int NMode, int MaxQLevel,
                        int DegrePol, int NPES, int Verbose, int NXDualHPlus,
                        double Mem, double KNNZ, double KNREZ, double KNZREZ,
                        int SumTarget, uint32_t TotalLFF,
                        uint32_t NXDualHTrunc) {
  // Return the maximal size of the active space B SizeActMax for a given memory
  // Mem(in megabytes),
  /*
  NXDualHTruncPos-1: Total number of raising excitations in H* (after truncation
  through ThrKX) NXDualHTrunc : Total number of excitations in H* (after
  truncation through ThrKX) SizeRezMax=SizeActMax*(NXDualHTruncPos-1)*KNREZ
  ...
  */
  ...
  Diff = (sizeof(I32) * (TotalLFF + NXDualHPlus) +
          sizeof(U8) * (NXDualHPlus * NMode) +
          2 * sizeof(UI) * NXDualHPlus); // LocalFF + Permuter
  Memoir -= Diff;
  if (Verbose) {
  ...
  Diff = sizeof(dble) * (DegrePol + 1 + NMode + 4) *
         ((MaxQLevel + DegrePol) * (MaxQLevel + DegrePol)); // QQ
  Memoir -= Diff;                                           // QQ
  if (Verbose) {
  ...
  Diff = sizeof(dble) *
         ((NMode * (NMode + 1)) * (NMode * (NMode + 1)) / 4); // Zeta
  Memoir -= Diff;                                             // Zeta
  if (Verbose) {
...
float EffectiveMemory2(char PidName[MaxChar]) {
  ...
  sprintf(
      Exec,
      "ps aux | grep %s | grep -v grep | awk 'BEGIN { sum=0 } {sum=sum+$6;} END\
 { print sum / 1024 }'",
      PidName);
  float Memoire = 0;
...
void ContribRot(uint8_t *Tester, int ni, int nj, int nk, int nl, int NMode) {
  // Increment by one each normal coordinate involved in rotational
  // coefficients. computation and return an equivalent excitation array in
  // Tester. Tester is the equivalent monomial c^(ijkl)= 1i+1j+1k+1l, (sum of
  // canonical vectors).
  InitTabInt(Tester, NMode) Tester[ni]++;
  Tester[nj]++;
  Tester[nk]++;
  Tester[nl]++;
}
int32_t AssignRot(uint8_t *Tester, int NMode, int ni, int nj, int nk, int nl) {
  // Store the equivalent monomial c^(ijkl)= 1i+1j+1k+1l, (sum of canonical
  // vectors) in Tester and return the equivalent number
  // nl*NMode^3+nk*NMode^2+nj*NMode+ni
  InitTabInt(Tester, NMode) Tester[ni]++;
  Tester[nj]++;
  Tester[nk]++;
  Tester[nl]++;
  return ni + NMode * nj + Powo<int32_t>(NMode, 2) * nk +
         Powo<int32_t>(NMode, 3) * nl;
}
...
void DeleteLFF(LocalFF &LFF) { delete[] LFF.Idx; }
